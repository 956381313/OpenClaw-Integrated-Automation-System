...
Write-Host "================================================" -ForegroundColor Cyan
Write-Host ("鏃堕棿: {0}" -f (Get-Date -Format 'yyyy-MM-dd HH:mm:ss')) -ForegroundColor Gray
Write-Host ""

# 妫€鏌?: 绯荤粺鍑嗗鐘舵€?Write-Host "妫€鏌?: 绯荤粺鍑嗗鐘舵€? -ForegroundColor Green
...
# 妫€鏌?: 绠＄悊鍛樻潈闄?Write-Host "妫€鏌?: 鏉冮檺鐘舵€? -ForegroundColor Green
...
# 妫€鏌?: 璁″垝浠诲姟鐘舵€?Write-Host "妫€鏌?: 璁″垝浠诲姟鐘舵€? -ForegroundColor Green
...
try {
    ...
    if ($task) {
        Write-Host ("  鉁?浠诲姟宸插瓨鍦? {0}" -f $task.TaskName) -ForegroundColor Green
        Write-Host ("    鐘舵€? {0}" -f $task.State) -ForegroundColor Gray
        Write-Host ("    鍚敤: {0}" -f $task.Enabled) -ForegroundColor Gray
        
        $taskInfo = Get-ScheduledTaskInfo -TaskName $taskName -ErrorAction SilentlyContinue
        if ($taskInfo) {
            Write-Host ("    涓婃杩愯: {0}" -f $taskInfo.LastRunTime) -ForegroundColor Gray
            Write-Host ("    涓嬫杩愯: {0}" -f $taskInfo.NextRunTime) -ForegroundColor Gray
        }
        ...
        if ($task.State -eq "Ready" -and $task.Enabled -eq $true) {
            ...
            Write-Host ("    鍚敤鍛戒护: Enable-ScheduledTask -TaskName `"{0}`"" -f $taskName) -ForegroundColor Gray
        }
    } else {
    ...
    Write-Host ("  鈩癸笍 鏃犳硶妫€鏌ヤ换鍔＄姸鎬? {0}" -f $_) -ForegroundColor Gray
}

Write-Host ""

# 妫€鏌?: 绯荤粺楠岃瘉鐘舵€?Write-Host "妫€鏌?: 绯荤粺楠岃瘉鐘舵€? -ForegroundColor Green
...
# 妫€鏌?: 鎵ц寤鸿
Write-Host "妫€鏌?: 鎵ц寤鸿" -ForegroundColor Green
...
Write-Host "鐘舵€佹憳瑕?" -ForegroundColor Green
Write-Host ("  绯荤粺鏂囦欢: {0}" -f $(if ($allFilesExist) { '鉁?瀹屾暣' } else { '鉁?涓嶅畬鏁? })) -ForegroundColor $(if ($allFilesExist) { "Green" } else { "Red" })
...
Write-Host ""
Write-Host "鐩稿叧鏂囨。:" -ForegroundColor Cyan
...
Write-Host ""
Write-Host "绠＄悊鍛戒护:" -ForegroundColor Cyan
...
Write-Host ""
Write-Host "鎻愮ず: 绯荤粺宸插畬鍏ㄩ獙璇侊紝鍙互瀹夊叏鎵ц璁剧疆" -ForegroundColor Green
