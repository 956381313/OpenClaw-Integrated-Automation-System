# OpenClaw Email Notification System (Fixed Version)
# Version: 1.0.1
...
# Main execution
# Handle command line parameters in a simpler way
$Type = $args[0]
$TestMode = $false
$DataJson = $null
...
for ($i = 0; $i -lt $args.Count; $i++) {
    switch ($args[$i]) {
        ...
        "-Help" { 
            ...
            Write-Host "  .\send-email-fixed.ps1 -Type <type> [-DataJson <json>] [-Test] [-Help]" -ForegroundColor Gray
            Write-Host ""
            ...
            Write-Host "  .\send-email-fixed.ps1 -Type BackupComplete -Test" -ForegroundColor Gray
            Write-Host "  .\send-email-fixed.ps1 -Type SecurityCheck -DataJson '{\"passed\":11,\"failed\":0}'" -ForegroundColor Gray
            exit 0
        }
    }
}
...
# Send the email
$result = Send-Email -Type $Type -Data $data -TestMode $TestMode
