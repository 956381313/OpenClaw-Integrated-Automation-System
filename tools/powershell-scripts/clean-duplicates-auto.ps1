# Auto Cleanup Script (non-interactive)
# Modified version that auto-confirms
...
Write-Host "=== OpenClaw Duplicate File Cleanup (Auto) ===" -ForegroundColor Cyan
Write-Host "Time: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')" -ForegroundColor Gray
Write-Host "Auto-confirm mode enabled" -ForegroundColor Yellow
Write-Host ""
...
$backupDir = "modules/duplicate/backup\cleanup-auto-$(Get-Date -Format 'yyyyMMdd-HHmmss')"
...
foreach ($group in $duplicateGroups) {
    ...
    foreach ($deleteFile in $deleteFiles) {
        Write-Host "  Delete: $($deleteFile.Name)" -ForegroundColor Red
...
Write-Host "Mode: AUTO-CONFIRM (files will be deleted)" -ForegroundColor Red
...
$cleanupReport = @"
OpenClaw Duplicate File Cleanup Report (Auto)
=============================================
...
---
Report generated by OpenClaw Duplicate File Cleanup v1.0.0 (Auto)
"@

# Save cleanup report
$cleanupReportFile = "modules/duplicate/reports\cleanup-auto-report-$(Get-Date -Format 'yyyyMMdd-HHmmss').txt"
